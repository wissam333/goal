    ...
    <template v-else>
      ...
      <div class="teams-grid">
        ...
        >
          ...
          <div
            class="card-header"
            :style="team.color ? `background: linear-gradient(135deg, ${team.color}22, ${team.color}08)` : ''"
          >
            <div
              class="card-logo"
            ...
            >
              ...
              />
              <span v-else class="logo-initial">{{ team.title?.charAt(0) }}</span>
            </div>
...
const { data: matchesData, pending: matchesPending } = useAsyncData("teams-matches", () => fetchMatches())
...
const summaryStats = computed(() => [
  {
    key: 'teams', label: 'stats.totalTeams', icon: 'mdi:shield-outline',
    value: teams.value.length, color: 'primary',
  },
  {
    key: 'played', label: 'standings.played', icon: 'mdi:soccer-field',
    value: matches.value.length, color: 'success',
  },
  {
    key: 'goals', label: 'stats.totalGoals', icon: 'mdi:soccer',
    value: matches.value.reduce((sum, m) => sum + (m.homeScore || 0) + (m.awayScore || 0), 0),
    color: 'warning',
  },
  {
    key: 'avg', label: 'stats.avgGoals', icon: 'mdi:chart-bar',
    value: matches.value.length
      ? (matches.value.reduce((s, m) => s + (m.homeScore || 0) + (m.awayScore || 0), 0) / matches.value.length).toFixed(1)
      : '0',
    color: 'info',
  },
]);
...
.logo-initial { font-size: 1.4rem; font-weight: 800; color: var(--primary); }
...
.form-dot {
  ...
  &.form-w { background: rgba(34,197,94,0.7); }
  &.form-d { background: rgba(148,163,184,0.5); }
  &.form-l { background: rgba(239,68,68,0.6); }
}
...
.wdl-sep { width: 1px; height: 24px; background: var(--border-color); }
.wdl-item {
  display: flex; flex-direction: column; align-items: center; gap: 2px;
  min-width: 36px;
}
.wdl-num { font-size: 1.1rem; font-weight: 800; }
.wdl-label { font-size: 0.65rem; color: var(--text-muted); text-transform: uppercase; }
.wdl-item.win   .wdl-num { color: #16a34a; }
.wdl-item.draw  .wdl-num { color: var(--text-muted); }
.wdl-item.loss  .wdl-num { color: #dc2626; }
...
.pts-value { font-size: 1.5rem; font-weight: 900; color: var(--primary); display: block; line-height: 1; }
.pts-label { font-size: 0.65rem; color: var(--primary); opacity: 0.7; text-transform: uppercase; }
