export const useLeagueData = () => {
  ...
  const fromSupabase = async (table, query = {}) => {
    ...
    if (query.order) q = q.order(query.order.field, query.order.dir ? { ascending: query.order.dir === "asc" } : {})
    if (query.limit) q = q.limit(query.limit)
    if (query.in) for (const { column, values } of query.in) q = q.in(column, values)
    const { data } = await q
    return data
  }
  ...
  const fetchTeam = async (slug) => {
    const d = await fromSupabase("teams", { eq: { slug }, select: "*", limit: 1 })
  ...
  const fetchPlayer = async (slug) => {
    const d = await fromSupabase("players", { eq: { slug }, select: "*", limit: 1 })
    return d?.length ? d[0] : null
  }
  ...
  const fetchMatches = async (filters = {}) => {
    ...
    if (filters.orderBy) query.order = { field: filters.orderBy.field, dir: filters.orderBy.dir || "asc" }
    if (filters.limit) query.limit = filters.limit
    if (filters.statusIn) query.in = [{ column: "status", values: filters.statusIn }]
  ...
  return { fetchTeams, fetchTeam, fetchPlayers, fetchPlayer, fetchMatches, fetchMatch, fetchSettings }
}
